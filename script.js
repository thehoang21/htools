...
function switchTab(tabName) {
    ...
    if (tabName === 'time') {
        ...
        initCountries();
    // Đã loại bỏ Mô Phỏng Vật Lý
    } else if (tabName === 'solar') {
        initSolarSystem();
    } else if (tabName === 'worldmap') {
        initWorldMap();
    // Đã loại bỏ Mô Phỏng Nhật/Nguyệt Thực
    ...
    list.innerHTML = countries.map((country, index) => {
        ...
        return `
            <div class="bg-white rounded-lg border border-slate-200 overflow-hidden">
                <div class="px-4 py-3 flex items-center gap-3 bg-slate-50/50 border-b border-slate-200">
                    <img src="${flag}" alt="${name}" class="w-12 h-8 object-cover rounded border border-slate-200 shrink-0">
                    <div class="flex-1">
...
// Accordion function removed - all country info now displayed by default
...
function updatePagination(totalItems, totalPages) {
    ...
    // Button size classes - smaller on mobile, with flex centering
    const btnSizeClasses = isMobile ? 'w-8 h-8 text-xs' : 'w-10 h-10 text-sm';
    const btnLayoutClasses = 'flex items-center justify-center';
    ...
    if (totalPages <= (isMobile ? 4 : 7)) {
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            pageNumbersHTML += `<button onclick="goToPage(${i})" class="${btnSizeClasses} ${btnLayoutClasses} rounded-lg border transition-colors touch-manipulation ${isActive ? 'bg-blue-600 text-white border-blue-600' : 'border-slate-200 bg-white hover:bg-slate-50 active:bg-slate-100'}">${i}</button>`;
        }
    } else {
        ...
        if (currentPage > showStart + 1 && currentPage < totalPages - showEnd) {
            ...
            pageNumbersHTML += `<button onclick="goToPage(${currentPage})" class="${btnSizeClasses} ${btnLayoutClasses} rounded-lg border transition-colors touch-manipulation bg-blue-600 text-white border-blue-600">${currentPage}</button>`;
            pageNumbersHTML += `<span class="${ellipsisSizeClasses} flex items-center justify-center text-slate-400">...</span>`;
        } else {
            pageNumbersHTML += `<span class="${ellipsisSizeClasses} flex items-center justify-center text-slate-400">...</span>`;
        }
...
// Đã loại bỏ toàn bộ logic mô phỏng vật lý
...
// Đã loại bỏ toàn bộ logic mô phỏng nhật/nguyệt thực
